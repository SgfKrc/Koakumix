...
| `S3.2-IMG-01` | S3.2 生图 | 独立 executor 生命周期、真实 diffusers/CUDA 适配边界和互斥锁 | RT-01 | **执行器与装配已落地（2026-09-15）**：`image/diffusers_executor.py`（惰性导入 torch/diffusers、**单加载器互斥锁**（8GB 卡不允许并发加载）、`status()` 观测、`auto/cuda/cpu` 由真实运行时解析、`close()` 幂等并释放显存、错误映射为稳定 code）+ `image/assembly.py`（**opt-in 装配**：`QLH_HARNESS_IMAGE_EXECUTOR=diffusers` 且运行时与已校验工件齐备才启用，否则显式 `unavailable` + 状态）。**工件齐备**：六个 SD 1.5 资产在 `H:\qlh_models\`（经主仓 `models/` junction 可达）；`sd15-original-v1` 为 diffusers 目录格式，`LocalImageEngine.inspect()` 实测 `valid=True`（16 文件、0 错误、0 缺失，与 `docs/已知问题记录.md:368` 记载的"原版 16/16"一致）。**唯一缺口 = `diffusers` 依赖未安装**（无需下载权重）；`.venv-qwen3-sidecar` 的 cu126 torch 已确认 `cuda True` |
