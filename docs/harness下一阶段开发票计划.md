...
**边界（如实说明）**：本机 `models/`、`_to_delete/`、`_archive/` 均无 SD 1.5 工件，所有 venv 也都没有 `diffusers`，因此**本轮没有做真机真跑**；`.venv-qwen3-sidecar` 的 cu126 torch 已确认 `cuda True`，一旦补上工件（约 4GB 下载），即可用 `QLH_HARNESS_IMAGE_EXECUTOR=diffusers` 装配真跑。用户裁决 2026-09-15：**先落地代码与接线，真机后置**。
